#!/usr/bin/env python3
"""
Start Node 3 (Miner Node) on port 8003
"""
...
def main():
    print("Starting DeSSIN Node 3 (Miner Node)")
    print("=" * 40)
    
    # Create configuration for Node 3
    config = DessinConfig.default()
    config.network.port = 8003
    ...
    try:
        success = node.start()
        if not success:
            print("Failed to start Node 3")
            return 1
        
        print("✓ Node 3 started successfully")
        ...
        peers = [8001, 8002, 8004]  # Connect to all other nodes
        for port in peers:
            try:
